...
function App() {
  ...
  const generatePPT = async () => {
    const {
      ...
      filename,
      rawPayload,
    } = pptForm.getFieldsValue()

    if (!endpoint?.trim()) {
      return message.warning('请输入PPT生成服务地址 endpoint')
    }
    setPptLoading(true)
    try {
      let body: any
      if (rawPayload && String(rawPayload).trim()) {
        ...
        body = {
          ...
          template: template || 'lecture.md.jinja'
        }
      }
      ...
      if (contentType.includes('application/json')) {
        ...
        if (data?.fileBase64) {
          ...
          downloadBlob(blob, filename || 'slides.pptx')
          message.success('PPT 生成成功，已开始下载')
          return
        }
        message.warning('服务返回JSON，但未包含 fileUrl 或 fileBase64')
        return
      }
        ...
        <Tabs
          ...
          >
            <Form.Item
              label="服务地址"
              name="endpoint"
              extra="你的PPT生成接口，如 http://localhost:5000/api/generate-ppt"
            >
              <Input placeholder="http://localhost:5000/api/generate-ppt"></Input>
            </Form.Item>
            <Form.Item label="鉴权" name="apiKey" extra="可选，放在Authorization头">
              <Input placeholder="Bearer xxx 或 自定义token"></Input>
            </Form.Item>
            <Form.Item label="标题" name="title">
              <Input placeholder="PPT 标题"></Input>
            </Form.Item>
            <Form.Item label="大纲" name="outline" extra="每行一条或自由文本">
              <Input.TextArea rows={4} placeholder={"- 主题A\n- 主题B"}></Input.TextArea>
            </Form.Item>
            <Form.Item label="模板" name="template">
              <Input placeholder="default 或你的模板标识"></Input>
            </Form.Item>
            <Form.Item label="文件名" name="filename">
              <Input placeholder="slides.pptx"></Input>
            </Form.Item>
            <Form.Item label="原始JSON" name="rawPayload" extra="可选，若填写将覆盖标题/大纲等上方字段">
              <Input.TextArea rows={6} placeholder='{"title":"...","outline":"...","template":"..."}'></Input.TextArea>
            </Form.Item>
          </Form>
          <Button loading={pptLoading} onClick={generatePPT} type="primary" block>
            生成 PPT
          </Button>
